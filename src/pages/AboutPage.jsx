...
/* ====== NEW: global smooth-ease for a silkier feel ====== */
const EASE_SMOOTH = [0.16, 1, 0.3, 1]; // smoother than the default; gentle accel/decel

// ------- motion helpers -------
const fadeUp = (d = 0) => ({
  initial: { opacity: 0, y: 16 },
  whileInView: { opacity: 1, y: 0 },
  transition: { duration: 0.7, delay: d, ease: EASE_SMOOTH }, // slightly longer, smoother
  viewport: { once: true, margin: '-80px', amount: 0.2 } // start a bit earlier, stable
});

const containerStagger = {
  initial: { opacity: 0 },
  whileInView: { opacity: 1 },
  transition: { staggerChildren: 0.1, ease: EASE_SMOOTH },
...
export default function AboutPage() {
  /* ====== NEW: ensure we start at the very top on first render ====== */
  useEffect(() => {
    // Jump immediately to top (avoids showing mid-scroll if user navigated back)
    window.scrollTo(0, 0);
    // Optional: if you prefer smooth scrolling from current position:
  ...
  return (
    /* ====== NEW: MotionConfig to softly standardize transitions across the page ====== */
    <MotionConfig transition={{ ease: EASE_SMOOTH }}>
      <div className="flex flex-col min-h-screen bg-gray-50 text-gray-900 dark:bg-gray-950 dark:text-gray-100">
        ...
        <header className="relative flex items-center justify-center h-[22rem] sm:h-[24rem] md:h-[26rem] pt-20 overflow-hidden">
          ...
          <motion.div
            aria-hidden
            className="absolute -top-24 -left-24 w-72 h-72 rounded-full bg-white/15 blur-3xl will-change-transform"
            initial={{ x: -40, y: -20, opacity: 0.6 }}
            animate={{ x: 10, y: 0, opacity: 0.75 }}
            transition={{ duration: 7, repeat: Infinity, repeatType: 'mirror', ease: EASE_SMOOTH }}
          />
          <motion.div
            aria-hidden
            className="absolute -bottom-28 -right-20 w-80 h-80 rounded-full bg-white/10 blur-3xl will-change-transform"
            initial={{ x: 30, y: 20, opacity: 0.5 }}
            animate={{ x: -10, y: 0, opacity: 0.65 }}
            transition={{ duration: 8.5, repeat: Infinity, repeatType: 'mirror', ease: EASE_SMOOTH }}
          />
            ...
            <h1 className="text-4xl sm:text-5xl md:text-6xl font-extrabold drop-shadow-lg tracking-tight">
              <span className="relative inline-block">
                ...
                <motion.span
                  ...
                  initial={{ x: '-120%' }}
                  animate={{ x: '120%' }}
                  transition={{ duration: 3.2, repeat: Infinity, ease: EASE_SMOOTH }}
                  style={{
                    background:
        ...
        <main className="flex-grow max-w-6xl mx-auto px-6 py-10 sm:py-14 space-y-20">
          ...
          <section className="grid md:grid-cols-2 gap-10 items-center">
            ...
            >
              <motion.div
                className="rounded-2xl p-6 sm:p-8 bg-white/70 dark:bg-white/10 backdrop-blur shadow-xl border border-white/60 dark:border-white/10 will-change-transform"
                whileHover={{ y: -6 }}
          ...
          <section>
            ...
            >
              {values.map((v) => (
                <motion.div
                  key={v.title}
                  className="flex flex-col items-center text-center p-6 bg-white dark:bg-gray-900 rounded-2xl shadow-lg border border-gray-100 dark:border-gray-800 will-change-transform"
                  variants={itemFade}
